...
public class EncryptionService {
  ...
  public String serve(String option) {
    ...
    if (eConnectorsConnectionInfos != null && !eConnectorsConnectionInfos.isEmpty()) {
      for (EConnectorsConnectionInfo eConnectorsConnectionInfo : eConnectorsConnectionInfos) {
        ...
        if (eMasterKeys != null && eCustomerKeys != null) {
          ...
          final String decryptedPass = encryptorAesGcm.decrypt(eConnectorsConnectionInfo.getPassword(), FIELD_CUSTOMER_ID);
          if (decryptedPass != null && !decryptedPass.isEmpty()) {
            ...
            final String encyptedText = encryptorAesGcm.encryptKeys(decryptedPass, FIELD_CUSTOMER_ID);
            if (encryptedText != null && !encryptedText.isEmpty()) {
              System.out.println("***********************");
              eConnectorsConnectionInfo.setPassword(encyptedText);
//              connectorsConnectionInfoDaoManager.saveConnection(eConnectorsConnectionInfo);
              logger.info("Password encrypted and saved : " + encyptedText);
              System.out.println("***********************");
            }
            else {
              System.out.println("***********************");
